//Forwarding distributed-LINE messages in the correct order using Lamport Algorithm
...
public class Lamport {
    ...
    class draw extends JFrame {
        ...
        public void paint(Graphics g) {
            ...
            for (i = 1; i <= p; i++) {
                go.drawLine(20, 100 * i, 900, 100 * i);
            }
            for (i = 1; i <= p; i++) {
                for (j = 1; j <= ev[i]; j++) {
                    k = i * 10 + j;
                    go.setPaint(Color.blue);
                    go.fillOval(100 * j, 100 * i - 3, 10, 10);
                    go.drawString("e" + i + j + "(" + en[i][j] + ")", 100 * j, 100 * i - 5);
                    h1 = hm.get(k);
                    if (h1 != 0) {
                        h11 = h1 / 10;
                        h12 = h1 % 10;
                        go.setPaint(Color.red);
                        drawArrow(go, 100 * h12 + 2, 100 * h11, 100 * j + 2, 100 * i);
                    }
                }
            }
    ...
    public void calc() {
        ...
        for (i = 1; i <= p; i++) {
            System.out.println("For person(process) : " + i);
            for (j = 1; j <= ev[i]; j++) {
                System.out.println("For instruction/step : " + (j));
